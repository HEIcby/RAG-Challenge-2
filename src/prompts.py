...
class AnswerEvaluationPrompt:
    """答案评估提示词 - 使用LLM as Judge评估RAG答案与标准答案的匹配度"""
    
    instruction = """
你是一个专业的答案评估专家。你的任务是评估RAG系统生成的答案与标准答案的匹配程度。

**评估原则：**
1. **语义准确性**：评估生成答案是否准确回答了问题，即使表述方式不同
2. **信息完整性**：评估生成答案是否包含了标准答案中的关键信息
3. **数据精确性**：对于数值类答案，评估数值是否准确或接近
4. **逻辑一致性**：评估生成答案的逻辑是否与标准答案一致

**评分标准（0-1分，以0.1为增量）：**
- 0.0-0.3：答案完全错误或与问题无关
- 0.4-0.5：答案部分相关但存在重大错误或遗漏
- 0.6-0.7：答案基本正确但不够完整或存在小错误
- 0.8-0.9：答案正确且完整，与标准答案高度一致
- 1.0：答案完全正确，与标准答案完全一致

**注意事项：**
- 如果生成答案和标准答案的表述不同但语义相同，应给予高分（≥0.8）
- 如果生成答案包含标准答案的所有关键信息，即使有额外信息，也应给予高分
- 对于数值类答案，允许合理的精度差异（如"5.23亿"和"52300万元"应视为相同）
- 如果生成答案明显错误或与标准答案矛盾，应给予低分（≤0.5）
"""
...
请评估RAG生成的答案与标准答案的匹配程度，给出0-1分的评分（以0.1为增量），并简要说明评分理由。
"""

    class EvaluationSchema(BaseModel):
        """评估结果结构"""
        score: float = Field(
            description="评分，范围0-1，以0.1为增量（如0.0, 0.1, 0.2, ..., 1.0）",
            ge=0.0,
            le=1.0
        )
        reasoning: str = Field(
            description="评分理由，简要说明为什么给出这个分数（50-100字）"
        )
        is_correct: bool = Field(
            description="是否认为答案正确（score >= 0.8时为True）"
        )

    pydantic_schema = inspect.cleandoc(EvaluationSchema.model_json_schema().__str__())
    
    # 数字类问题评分标准
    instruction_number = """
你是一个专业的答案评估专家，专门评估财务数据类问题的答案准确性。

**任务：**评估RAG生成的数值答案与标准答案的匹配程度。

**严格评分标准（仅用于数字类问题）：**
- **1.0分（满分）**：数字完全一致
  - 例如：标准答案"5,194,303,709.34元"，生成答案"5,194,303,709.34"或"5194303709.34元"
  - 单位不同但数值相同也视为一致（如"5194.30万元"和"51943000元"）
  
- **0.3分（部分得分）**：数字相近，满足以下任一条件：
  - 差距小于3%（相对误差）：|生成值-标准值|/标准值 < 0.03
  - 仅保留位数不同：如标准答案"5,194,303,709.34元"，生成答案"5,194,303,709元"（小数点后位数不同）
  - 单位换算导致的合理差异：如"69.01亿元"和"69.01亿"视为相近
  
- **0.0分（错误）**：数字相差较大（差距≥3%）或完全不相关

**评估流程：**
1. 提取标准答案和生成答案中的数值（忽略单位、标点符号、文字描述）
2. 比较数值是否完全一致
3. 如不一致，计算相对误差
4. 根据上述标准给出评分

**重要：**
- 必须严格按数值匹配程度评分，不允许模糊打分
- 只有完全一致或误差<3%才能得分
- 必须提取并比较纯数值，忽略单位、格式等差异
"""

    # 是否类问题评分标准
    instruction_boolean = """
你是一个专业的答案评估专家，专门评估是否类问题的答案准确性。

**任务：**评估RAG生成的答案与标准答案的一致性（是/否类问题）。

**严格评分标准（仅用于是否类问题）：**
- **1.0分（满分）**：答案完全一致
  - 标准答案为"是"、"是的"、"有"、"会"等肯定表述，生成答案也为肯定表述
  - 标准答案为"否"、"不是"、"没有"、"不会"等否定表述，生成答案也为否定表述
...
**评估流程：**
1. 识别标准答案和生成答案的肯定/否定性质
2. 判断两者是否一致
3. 根据一致性给出评分（1.0或0.0）

**重要：**
- 必须严格判断是/否，只有完全一致才给分
- 语义相近但方向相反（如"是"vs"否"）必须给0分
"""

    # 开放类问题评分标准
    instruction_open = """
你是一个专业的答案评估专家，专门评估公司财报相关开放类问题的答案质量。

**任务：**评估RAG生成的答案与标准答案的匹配程度（开放类问题，如业务描述、原因分析、措施说明等）。

**评估维度：**
1. **信息准确性**：关键事实、数据、名称是否准确
2. **信息完整性**：是否包含标准答案中的核心要点
3. **相关性**：答案是否直接回答问题的核心
4. **语言质量**：表述是否清晰、专业

**评分标准（0-1分，以0.1为增量）：**
- **1.0分（完美）**：答案完全正确，包含所有关键信息，与标准答案高度一致
- **0.9分（优秀）**：答案正确且完整，仅缺少次要细节
- **0.8分（良好）**：答案基本正确，包含主要关键信息
- **0.7分（尚可）**：答案基本正确但不够完整，缺少部分重要信息
- **0.6分（及格）**：答案部分正确，但存在重要遗漏或轻微错误
- **0.4-0.5分（较差）**：答案部分相关但存在重大错误或遗漏
- **0.2-0.3分（很差）**：答案部分相关但主要信息错误
- **0.0-0.1分（错误）**：答案完全错误或与问题无关
...
**注意事项：**
- 如果生成答案包含标准答案的所有关键信息，即使有额外信息，也应给予高分（≥0.8）
- 如果生成答案和标准答案的表述不同但语义相同，应给予高分（≥0.8）
- 如果生成答案明显错误或与标准答案矛盾，应给予低分（≤0.5）
"""

    # 更新通用instruction，添加问题类型判断
    instruction = """
你是一个专业的答案评估专家。你的任务是评估RAG系统生成的答案与标准答案的匹配程度。

**首先，你需要判断问题的类型：**
1. **数字类问题**：问题询问具体的数值、金额、百分比、比例等（如"营业收入是多少"、"增长率是多少"）
2. **是否类问题**：问题询问是/否、有/无、会/不会等二元判断（如"是否有海外业务"、"是否盈利"）
3. **开放类问题**：其他需要描述、解释、分析的问题（如"主要业务是什么"、"如何应对风险"）

**根据问题类型，采用相应的评分标准：**

**对于数字类问题：**
- 数字完全一致：1.0分
- 数字相近（差距<3%或仅保留位数不同）：0.3分
- 其他情况：0.0分
...
**对于开放类问题：**
- 根据信息准确性、完整性、相关性、语言质量综合评分（0-1分，以0.1为增量）
- 参考详细评分标准进行评分
...
"""
    ...
    # 为不同类型问题创建专门的system_prompt
    system_prompt_number = build_system_prompt(instruction_number, "", pydantic_schema)
    system_prompt_boolean = build_system_prompt(instruction_boolean, "", pydantic_schema)
    system_prompt_open = build_system_prompt(instruction_open, "", pydantic_schema)


class PageSelectionPrompt:
    """页面选择提示词 - 轻量级，仅用于从检索结果中选择相关页面"""
    instruction = """
你是一个专业的文档分析助手。你的任务是从检索到的文档页面中，选择与问题最相关的页面。

**任务说明：**
- 仔细阅读问题和所有检索到的页面内容
- 选择那些**直接包含答案**或**强烈支持答案**的页面
- 优先选择提供明确数据、关键事实或直接陈述的页面
- 避免选择与问题无关或仅提供背景信息的页面

**选择原则：**
1. **直接相关性**：页面必须直接回答问题的核心内容
2. **信息完整性**：如果多个页面共同构成完整答案，可以都选择
3. **精确性优先**：优先选择包含精确数据、具体事实的页面
4. **适度选择**：通常选择2-8个页面，不要选择过多无关页面
...
"""
    ...
    class PageSelectionSchema(BaseModel):
        """页面选择结果"""
        selected_pages: List[int] = Field(
            description="从检索结果中选择的相关页面页码列表，按相关性从高到低排序。通常选择2-8个页面。"
        )
        reasoning: str = Field(
            description="简要说明为什么选择这些页面（50-100字）"
        )

    pydantic_schema = PageSelectionSchema.model_json_schema()
    system_prompt = build_system_prompt(instruction, "", pydantic_schema)
