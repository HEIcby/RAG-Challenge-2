...
def load_canonic_answers():
    ...
    for submission in load_submissions():
        ...
        # 只测试前 60 题
        schema_items = list(schemas.items())[:60]

        for q, data in schema_items:
            ...
            if len(data.reference_pools) == 0 and len(predicted_refs) == 0:
                ...
                for proof_needed in data.reference_pools:
                    found_proof = len(set(predicted_refs).intersection(proof_needed)) > 0
                    if not found_proof:
                        max_ref_score -= 0.25
    ...
    for i, r in enumerate(rankings):
        ...
        accuracy = 100.0 * r.val_score / max(1, (60 - r.no_rank))  # 除以前60题
