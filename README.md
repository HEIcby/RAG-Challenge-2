...
使用金盘科技数据集 (`data/val_set/`):

```bash
cd data/val_set/

# 步骤1: 解析PDF (需要较长时间)
python ../../main.py parse-pdfs --max-workers 3

# 步骤2: 创建向量数据库
python ../../main.py process-reports
```

**注意**: 如果已有处理好的数据库，跳过此步骤。
...
streamlit run app_jinpan_qa.py --server.port 8501 --server.address 0.0.0.0
```
